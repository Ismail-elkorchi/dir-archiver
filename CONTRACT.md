...
`tgz` and `tar.gz` identify the same format family. The exact alias returned by automatic inference can depend on whether the format came from a write destination, a read filename, or an explicit `format` option.
...
- Normalization preserves the opened source format; the destination extension does not convert formats.
- The result contains the source format and a versioned bytefold normalization report.
- A reader without normalization support causes `DIRARCHIVER_NORMALIZE_UNSUPPORTED`.

Normalization is not transactional and can leave a partial destination after failure.
